...
process *crear_Proceso(int _id, int _delay, char *_nombreProceso, FN_ACCION _proceso_realizar, int _estado){
	...
	pthread_create(&_p->crear_Proceso,NULL,_proceso_realizar,NULL);
	_p->estado = _estado;
	return _p;
}
...
void ejecutar_Procesos(array_procesos *_array){
	...
	for(int i = 0; i < cont_procesos; i++){
		if(_array[i].proceso.estado == ACTIVO){
			printf("<=================================================================================>\n");
			printf("\tEl proceso de nombre [%s], con ID [%d], se encuentra realizando su [PROCESO], con un delay de [%d] segundo(s)\n", _array[i].proceso.nombrePROCESO, _array[i].proceso.id, _array[i].proceso.delay),_array[i].proceso.proceso_realizar();
			
			//pthread_create(&_array[i].proceso.proceso_Creado,NULL,_array[i].proceso.proceso_realizar,NULL); //Creando hilo
			//pthread_join(_array[i].proceso.crear_Proceso,NULL); //Aqui se ejecuta el hilo
